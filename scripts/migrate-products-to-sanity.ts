/**
 * One-off migration script to upsert `src/data/products.json` into Sanity.
 * Usage: set `SANITY_PROJECT_ID`, `SANITY_DATASET`, `SANITY_WRITE_TOKEN` in env, then run:
 *   node ./scripts/migrate-products-to-sanity.ts
 *
 * This script is a convenience - it is not executed automatically by the app.
 */
...
async function upsert() {
  const mutations = products.map((p: any) => {
    ...
    const doc: any = { _type: 'product', name: p.name, price: p.price, description: p.description, category: p.category };
